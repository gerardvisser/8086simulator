...
Statement::Statement (Type type, std::vector<std::shared_ptr<Token>>& tokens, std::vector<Operand>& operands) :
    m_type (type), m_tokenCount (tokens.size ()), m_operandCount (operands.size ()), m_size (0), m_location (-1) {
...
int Statement::location (void) const {
  return m_location;
}

void Statement::location (int val) {
  m_location = val;
}
